...
/*
	...
	mpz_init_set_str(b,"abcdef",16);
	mpz_init(a);
	//gmp_printf("b = %Zd\n",b);
	mpz_fdiv_q_2exp(a,b,24-4);
	gmp_printf("a = %Zx\n",a);
	exit(-1);
*/
	/***********************************************************************/
		...
		for (struct_i = 0; struct_i < __NB_STRUCTURES__; struct_i++) // hash test
		{
			if(structs[struct_i] == 1)
			{
				if(!world_rank)
				{
					...
					pcs_mu_init_server(P, Q, E, large_prime, A, nb_bits, trailling_bits, struct_i, nb_threads, level, world_size);
				}
				else
				{
					pcs_mu_init_client(P, Q, E, large_prime, A, nb_bits, trailling_bits, nb_threads, level);
				}
				if(world_rank)
				{
					pcs_mu_run_order_client(nb_threads, world_rank);
				}
				else
				{
					gettimeofday(&tv1,NULL);
					//pcs_mu_run(x, nb_threads, nb_collisions);
					pcs_mu_run_order_server(xs,nb_threads,times,pts_per_users);
